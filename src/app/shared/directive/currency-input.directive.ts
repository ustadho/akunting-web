...
export class CurrencyInputDirective implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    this.el.value = this.currencyPipe.transform(this.el.value, ' ', '', '1.0-0');
    const ctrl = this.ngControl.control;
    this.subscription = ctrl.valueChanges
      .subscribe(res => {
        let newValue = typeof res === "string" ? res.replace(/[^0-9.]+/g, '') : res;

        // console.log(newValue.toString());

        // setTimeout(() => {
        //   this.el.value = this.currencyPipe.transform(newValue, ' ');
        // }, 500)
      })
  }
  ...
  // onChange(value) {
  //   // on blur, add currency formatting
  //   let newValue = typeof value === "string" ? value.replace(/[^0-9.]+/g, '') : value;
